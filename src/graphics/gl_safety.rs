...
impl SafeGL {
    ...
    pub fn check_error_with_context(context: &str) -> Result<(), MiniquadError> {
        Self::check_error()
            .map_err(|e| MiniquadError::GraphicsApi(GraphicsApiError::OpenGL(e)))
            .map_err(|e| {
    ...
    pub fn gen_buffers(n: i32) -> Result<Vec<GLuint>, MiniquadError> {
        ...
        if n > MAX_BUFFERS {
            return Err(MiniquadError::InvalidParameter(format!(
                "Cannot generate more than {} buffers at once",
                MAX_BUFFERS
            )));
        }
    ...
    pub fn gen_textures(n: i32) -> Result<Vec<GLuint>, MiniquadError> {
        ...
        if n > MAX_TEXTURES {
            return Err(MiniquadError::InvalidParameter(format!(
                "Cannot generate more than {} textures at once",
                MAX_TEXTURES
            )));
        }
    ...
    pub fn create_shader(shader_type: GLenum, source: &str) -> Result<GLuint, MiniquadError> {
        ...
        let c_source = std::ffi::CString::new(source).map_err(|_| {
            MiniquadError::InvalidParameter("Shader source contains null bytes".to_string())
        })?;
